#!/usr/bin/env bash
# =============================================================================
# onlinker - ZeroTier 离线安装脚本
# =============================================================================
# 功能：Ubuntu 离线安装 zerotier-one
...
if grep -q $'\r' "$0" 2>/dev/null; then
  err "检测到 CRLF。先执行：sed -i 's/\r$//' '$0'"
  exit 1
fi
if [ "$(id -u)" -ne 0 ]; then
  err "请使用 root 权限运行此脚本"; exit 1
fi
...
PKG_ROOT="${SCRIPT_DIR}/packages/zerotier/deb"
ALLOW_REINSTALL="${ALLOW_REINSTALL:-0}"   # 或传参 --reinstall
...
detect_ubuntu(){
  ...
  [ -n "${CODENAME:-}" ] || { err "无法识别 Ubuntu 代号（xenial/bionic/focal/jammy/noble）。请检查 /etc/os-release 文件。"; exit 2; }
  case "$(uname -m)" in
    x86_64) ARCH_DEB=amd64 ;;
    aarch64|arm64) ARCH_DEB=arm64 ;;
    *) err "不支持的架构：$(uname -m)（仅支持 amd64/arm64）"; exit 2 ;;
  esac
  log "检测到系统：Ubuntu ${VERSION_ID:-?} (${CODENAME}) ($(uname -m) -> ${ARCH_DEB})"
}
...
ensure_net_tools(){
  ...
  if [ ${#missing_tools[@]} -gt 0 ]; then
    err "离线模式：缺少必要工具：${missing_tools[*]}"
    err "请手动安装：apt-get install ${missing_tools[*]}"
    err "或使用在线模式：./onlinker.sh --online"
    exit 1
  fi
...
pick_deb(){
  local dir="${PKG_ROOT}/${CODENAME}"
  [ -d "$dir" ] || { err "缺少离线包目录：$dir（请先按清单放入离线包）"; exit 3; }
  shopt -s nullglob
  local files=("$dir"/zerotier-one_*_"${ARCH_DEB}".deb)
  shopt -u nullglob
  [ ${#files[@]} -gt 0 ] || { err "未找到离线包：$dir/zerotier-one_*_${ARCH_DEB}.deb"; exit 3; }
  DEB_PATH="$(printf '%s\n' "${files[@]}" | sort -V | tail -n1)"
  ok "找到离线包：$DEB_PATH"
}

# 关键修复：只认 install ok installed
is_installed(){
  dpkg-query -W -f='${Status}\n' zerotier-one 2>/dev/null | grep -qx 'install ok installed'
}
...
start_enable_service(){
  ...
  if ! systemctl is-active --quiet zerotier-one 2>/dev/null; then
    if command -v service >/dev/null 2>&1; then
      ...
      /etc/init.d/zerotier-one start 2>/dev/null || true
    fi
  fi

  # 兜底验证
  if systemctl is-active --quiet zerotier-one 2>/dev/null \
     || pgrep -x zerotier-one >/dev/null 2>&1; then
    ok "服务已运行：zerotier-one"
  else
    err "服务未处于运行状态。请查看：journalctl -u zerotier-one 或 /etc/init.d/zerotier-one status"
    exit 5
  fi
}
...
show_token_hint(){
  ...
  [ -s "$t" ] && ok "控制器 token：$t" || warn "尚未生成 token：$t（服务刚启动时稍等片刻）。"
}
...
deploy_moon(){
  ...
  # 5. 进入安装目录
  cd /var/lib/zerotier-one/ || { err "无法进入 ZeroTier 目录"; exit 1; }
